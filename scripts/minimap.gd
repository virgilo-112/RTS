...
# --------- World data --------- #
@onready var camera_main: Camera2D = get_tree().current_scene.get_node("LocalPlayer/CameraMain")
...
# --------- Define rectangle inside minimap --------- #
...
func update_camera_rect() -> void:
	...
	camera_rect.size = Vector2(visible_world_size.x / map_world_rect.size.x * minimap_size.x,visible_world_size.y / map_world_rect.size.y * minimap_size.y)
	var normalized_pos := (camera_main.global_position - map_world_rect.position) / map_world_rect.size
	camera_rect.position = (normalized_pos * minimap_size- camera_rect.size / 2.0)
